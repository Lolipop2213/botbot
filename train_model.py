# train_model.py
"""
Обновленный скрипт для обучения модели на расширенном датасете.
"""
import warnings
import logging
...
def main():
    # 1) Загружаем данные
    df = pd.read_csv("enhanced_training_dataset.csv") # <-- ИЗМЕНЕНО
    
    # Проверка и преобразование 'entry_time' (теперь она должна быть в файле)
    if 'entry_time' not in df.columns:
        raise ValueError("Колонка 'entry_time' отсутствует в enhanced_training_dataset.csv. Она необходима для TimeSeriesSplit.")
